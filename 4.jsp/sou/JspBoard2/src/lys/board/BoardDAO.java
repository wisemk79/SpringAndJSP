...
public class BoardDAO { //MemberDAO
	...
	//추가2(10개씩 끊어서 담아서  출력(검색어에 해당되는 데이터를 찾아서)///
public List<BoardDTO> getBoardArticles(int start,int end,
		...
		try {
			...
			System.out.println("getBoardArticles()의 sql구문=>"+sql);//오타확인
			//--------------------------------------------------------------------------
			pstmt=con.prepareStatement(sql);
			pstmt.setInt(1, start-1);//mysql은 레코드순번이 내부적으로 0부터 시작
			pstmt.setInt(2, end);//몇개씩 끊어서 보여줄것인지를 지정
			...
			System.out.println("getBoardArticles() 메서드 오류발생=>"+e);
		}finally {
			pool.freeConnection(con,pstmt,rs);
		}
		return articleList;
	...
	/////추가(페이징 처리를 재조정해주는 메서드 작성(ListAction 클래스에서 불러오기)/////////////////////////////////////////////////////////////////
 	//1.화면에 보여주는 페이지 번호   2.화면에 출력할 레코드 갯수
public Hashtable  pageList(String pageNum,int count) {
		...
		// 총페이지수,시작,종료페이지 계산(list.jsp에서 가져온 경우)
		...
		// 페이징처리에 대한 계산결과->Hashtable->ListAction에 전달->request저장(공유)->list.jsp
